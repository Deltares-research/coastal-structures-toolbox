...
@pytest.mark.parametrize(
    ...
    (
        ([2.0, 2.0, 0.45, 0.15, 1.5, 6.0, 2.0, 0.7, 1.2, 1000]),
        ([1.5, 1.5, 0.60, 0.20, 1.5, 6.0, 2.0, 0.7, 1.2, 10000]),
    ),
)
def test_internal_consistency_S_Dn50(
    cot_alpha, cot_phi, gamma, Dn50, Hs, Tmm10, Rc, Rc2_front, Rc2_rear, N_waves
):
    S_result = vangent2007.calculate_damage_number_S(
        cot_alpha=cot_alpha,
        cot_phi=cot_phi,
        gamma=gamma,
        Dn50=Dn50,
    ...
    Dn50_result = vangent2007.calculate_nominal_rock_diameter_Dn50(
        cot_alpha=cot_alpha,
        cot_phi=cot_phi,
        gamma=gamma,
        S=S_result,
    ...
    assert Dn50_result == pytest.approx(
        Dn50, abs=1e-2
    )  # TODO: check tolerance, now cm accuracy
